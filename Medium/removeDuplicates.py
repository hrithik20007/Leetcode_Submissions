'''
Given an integer array nums sorted in non-decreasing order, remove some duplicates in-place such that each unique element appears at most twice. The relative order 
    ...
    def removeDuplicates(self, nums: List[int]) -> int:
        ...
        while(l<len(nums)):
            if nums[l]==nums[l-1]==nums[l-2]:
                nums.pop(l)                             #Adding '_' or anything else at the end is not necessary
                l-=1                                    #We do l-=1 so as to check the current l's element as well (after l+=1) which is actually the l+1's element 
                                                        #before popping l's element
            l+=1
