...
/*
Logic: First we calculate the length of the two lists. The starting pointer of the longer node is brought to an
intermediate node such that both the pointers are at a equal distance from their lists' end. Then we traverse
the lists by one node. If at a point, both the pointers point to the same node, it means that an intersection
has been found. 
*/
